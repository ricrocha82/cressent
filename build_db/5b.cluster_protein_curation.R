...
write_fasta_by_cluster <- function(df, family, output_dir) {
    ...
    df %>%
        ...
        group_walk(~ writeLines(
            paste0(">", .x$protein_description, "\n", .x$protein_sequence),
            file.path(output_dir, paste0(family,"_",.y$cluster, ".fa"))
            ))
}


# function ot aovid error while reading file
safely_read_data <- function(file_path) {
...
process_family <- function(family, dir, dir_path_ann, dir_path_unann) {
  ...
  df_longer = safely_read_data(paste0(dir,"/mcl/",family,"_protein_description.csv"))

  if (!is.null(df_longer)) {
  # filter annotated proteins
    ...
    clusters_to_keep_rep <- df_rep[, .N, by = cluster][N >= 8, cluster]
